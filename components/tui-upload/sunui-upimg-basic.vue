		...
		const upload_task = await uni.uploadFile({
			...
			async success(res) {
				if (res.statusCode == 200) {
					...
					console.log(`%c 后端上传(成功返回地址):${data.data.path}`, 'color:#1AAD19');
					// 先获取图片url(各个后端返回值不一，所以造成出入)
					// 修改获取的图片返回值路径
					// 提示：data.info改为你图片返回地址即可
					upload_picture_list[j]['path_server'] = data.data.path; //修改返回图片地址即可 => 无论是腾讯云还是阿里云或者其它(只要返回图片地址以及引入对应的js文件进行一些配置即可扩展本插件)
...
<style scoped>
	...
	@import url("../../static/icon/iconfont-remove.css");
	...
	*/
	...
	.icon-addicon {
		/* #ifdef H5 */
		background: url('../../static/icon/icon-up.svg') no-repeat;
		/* #endif */
		/* #ifndef H5 */
		background: url('../../static/images/caseBox/icon-up.png') no-repeat;
		/* #endif */
	...
	.icon-card {
		/* #ifdef H5 */
		background: url('../../static/icon/card.svg') no-repeat;
		/* #endif */
		/* #ifndef H5 */
	...
	.icon-certificate {
		/* #ifdef H5 */
		background: url('../../static/icon/certificate.svg') no-repeat;
		/* #endif */
		/* #ifndef H5 */
		background: url('https://www.playsort.cn/file/icon-up.svg') no-repeat;
		/* #endif */
	...
	/* demo */
	.sunsin_picture_list uni-view:first-child{
  margin: 0 auto;
}
</style>
